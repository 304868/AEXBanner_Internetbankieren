...
public class BalieServer extends Application {
    ...
    public boolean startBalie(String nameBank) throws RemoteException, NotBoundException {
            ...
            try {
                ...
                else if (nameBank.equals("RaboBank"))
                {
                    port = 1101;
                }
                else if (nameBank.equals("ABN AMRO"))
                {
                    port = 1102;
                }  
                else if (nameBank.equals("ASN"))
                {
                    port= 1103;
                }
                ...
                r.rebind(nameBank, balie);
                Registry registry = java.rmi.registry.LocateRegistry.getRegistry("127.0.0.1",1201);
                ICentraleBank cb = (ICentraleBank)registry.lookup("centrale");
                cb.addBalie(balie);
                return true;
